...
def get_student_profile(student_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    overall_health = concept_mastery.get("Algebra", 50.0)
    ...
    return {
        ...
        "trend": "Improving" if overall_health >= 60 else "Requires Focus",
        "concept_mastery": concept_mastery,
        "recent_exams_count": exams_count,
        "resolved_weaknesses": ["Basic Expressions"] if overall_health >= 70 else [],
        "active_root_causes": active_root_causes or (["Weak Algebraic Manipulation"] if overall_health < 75 else [])
    }
