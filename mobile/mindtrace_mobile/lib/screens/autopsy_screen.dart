...
class _AutopsyScreenState extends State<AutopsyScreen> {
  ...
  Future<void> _loadAnalysis() async {
    try {
      ...
      setState(() {
        _errorMsg = "Error loading exam analysis: $e";
        _isLoading = false;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return Scaffold(
        ...
        appBar: AppBar(title: const Text("Exam Autopsy"), backgroundColor: const Color(0xFF1E293B)),
    ...
    final score = (_analysis?["score"] as num?)?.toDouble() ?? 0.0;
    final maxScore = (_analysis?["max_score"] as num?)?.toDouble() ?? 100.0;
    final incorrectCount = _analysis?["incorrect_count"] ?? 0;
    final rootCause = _analysis?["root_cause"] ?? "Weak Algebraic Manipulation";
    final confidence = (_analysis?["confidence"] as num?)?.toDouble() ?? 0.91;
    final summary = _analysis?["summary"] ?? "Identified recurring error patterns.";
    final evidence = List<String>.from(_analysis?["evidence"] ?? []);
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                children: [
                  ...
                  Text("$incorrectCount Incorrect Answers Identified", style: const TextStyle(color: Colors.amberAccent, fontSize: 13, fontWeight: FontWeight.w500)),
                ],
              ),
            ),
            ...
            Row(
              children: [
                _buildErrorCard("Concept errors", "${_analysis?["error_counts"]?["concept_errors"] ?? 0}", Colors.purpleAccent),
                const SizedBox(width: 8),
                _buildErrorCard("Calculation errors", "${_analysis?["error_counts"]?["calculation_errors"] ?? 0}", Colors.orangeAccent),
                const SizedBox(width: 8),
                _buildErrorCard("Procedural errors", "${_analysis?["error_counts"]?["procedural_errors"] ?? 0}", Colors.blueAccent),
              ],
            ),
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Text(
                            "${(confidence * 100).toInt()}% diagnostic confidence",
